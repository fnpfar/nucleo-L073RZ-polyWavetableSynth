...
uint8_t flag_first_DAC_half_buffer = 0;
...
int main(void) {
	...
	uint16_t dac_dma_buffer[DAC_BUFFER_SAMPLES];
	for (int n = 0; n < DAC_BUFFER_SAMPLES; n++) { // generates a 12-bit sine wave, 2 periods
		double sinn = sin(2 * (2 * M_PI * n) / DAC_BUFFER_SAMPLES);
	...
	DAC_BUFFER_SAMPLES, DAC_ALIGN_12B_R);
	...
	uint64_t next_half_buffer_to_write = 0;
	while (1) {

		if (flag_first_DAC_half_buffer == 1) { // first half buffer transfer complete
			flag_first_DAC_half_buffer = 0; // resets flag
			// update first half buffer samples
			for (int n = 0; n < DAC_BUFFER_SAMPLES / 2; n++) {
				dac_dma_buffer[n] = (uint16_t) (dac_dma_buffer[n] * 0.96);
			}
			next_half_buffer_to_write++;

		}
		if (flag_second_DAC_half_buffer == 1) { // second half buffer transfer complete
			flag_second_DAC_half_buffer = 0;  // resets flag
			// update second half buffer samples
			for (int n = DAC_BUFFER_SAMPLES / 2; n < DAC_BUFFER_SAMPLES; n++) {
				dac_dma_buffer[n] = (uint16_t) (dac_dma_buffer[n] * 0.96);
			}
			next_half_buffer_to_write++;
		}
